$(document).ready(function () {
    $("#AddNewBook").click(function (event) {
                ...
                $("#dialog-add-form").dialog(
                              {
                                  ...
                                  buttons: {
                                   ...
                                   allFields = $([]).add(name).add(classlevel).add(picture), tips = $(".form_msgs");
                                          ...
                                          bValid = bValid && checkRegexp(name, /^[a-zA-Z0-9\s]+$/, "Name Of The Book should consist of A to Z , 0 to 9 letters.");
                                          bValid = bValid && checkNull(classlevel, "Please select a class level ");
                                          bValid = bValid && minLength(picture, "Please select a photo for the cover of the book ! ", 1);
    ...
    $(".UpdateExistingBook").click(function (event) {
        event.preventDefault();
        $.ajax({
            url: $(this).attr('href'),
            success: function (temp) {
    ...
    $("#ChangeSettingsBook").click(function (event) {
        event.preventDefault();
        $.ajax({
            url: $(this).attr('href'),
            success: function (temp) {
                ...
                $("#dialog-add-form").dialog(
                              {
                                  ...
                                  buttons: {
                                      "Update": function () {
                                          ...
                                          var bValid = true;
                                          allFields.removeClass("form_error_alrt");
                                          bValid = bValid && checkNull(publish, "Please choose an option to publish this news or update !  ");
                                          if (bValid) {
                                              $('#dialog-add-form Form').submit();
                                          }
                                      },
